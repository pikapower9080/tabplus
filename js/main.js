...
function processSubSets() {
    document
        .querySelectorAll("fieldset.subset[data-requires]")
        .forEach((subset) => {
            if (subset.dataset.requires) {
                if (
                    document.getElementById(subset.dataset.requires) &&
                    document.getElementById(subset.dataset.requires).type ==
                        "radio"
                ) {
                    if (
                        document.getElementById(subset.dataset.requires).checked
                    ) {
...
function updateFontFamily() {
    ...
    if (familyText) {
        ...
        document.documentElement.style.setProperty(
            "--font",
            `'${el.fontFamily.value}', 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif`,
        );
        document.documentElement.style.setProperty(
            "--secondary-font",
            `'${el.secondaryFont.value}', sans-serif`,
        );
    }
}
...
if (get("mg-option")) {
    if (document.querySelector(`input[value=${get("mg-option")}]`)) {
        document.querySelector(`input[value=${get("mg-option")}]`).checked =
            true;
    }
} else {
    g("mg-option-none").checked = true;
}
if (get("font-family")) {
...
document.getElementById("reset-btn").addEventListener("click", () => {
    if (confirm("Are you sure?")) {
        localStorage.removeItem("tabplus");
        save = {};
            ...
            file.text().then((content) => {
                try {
                    ...
                    if (content) {
                        if (
                            "tabplus" in content &&
                            typeof content.tabplus == "object"
                        ) {
                            localStorage.setItem(
                                "tabplus",
                                JSON.stringify(content.tabplus),
                            );
                            window.location.reload();
                        } else {
                            alert(
                                "The selected file isn't a valid tab+ backup.",
                            );
                        }
                    }
                } catch (error) {
                    console.error(error);
                    alert(
                        "The selected file contained a syntax error, or there was another problem loading the backup.",
                    );
                }
            });
        } else {
            alert(
                "The selected file isn't saved in the right format, it should be a JSON file.",
            );
        }
    });
    input.click();
        ...
        if (chrome && "storage" in chrome) {
            chrome.storage.sync
                ...
                .catch((error) => {
                    console.error(error);
                    alert(
                        `Failed to backup data, it's possible that you've reached the sync data limit. Try again in a few minutes or consider backing up to a file using the "Save Backup" option.`,
                    );
                });
        } else {
            alert(
                "You need to be running in an extension environment for sync backup to work.",
            );
        }
    }
});
            ...
            chrome.storage.sync
                ...
                .then((data) => {
                    if ("backup" in data && typeof data.backup == "object") {
                        ...
                        alert(
                            "No backup data was found, it was corrupted, or the backup was created in an incompatible version of Tab+",
                        );
                    }
                })
                .catch((error) => {
                    console.error(error);
                    alert(
                        `Failed to restore backup data, it's possible that you've reached the sync data limit. Try again in a few minutes.`,
                    );
                });
        } else {
            alert(
                "You need to be running in an extension environment for sync restore to work.",
            );
        }
    }
});
...
if (!saved("first-time")) {
    g("welcome").showModal();
    setTimeout(() => {
        document.querySelector("#welcome > .close-button").disabled = false;
    }, 1); // Prevents the close button from being autofocused
    setTimeout(() => {
        document.querySelector("#welcome > .settings-button").disabled = false;
    }, 1);
    document
        .querySelector("#welcome > .close-button")
        .addEventListener("click", () => {
            set("first-time", false);
            g("welcome").close();
        });
    document
        .querySelector("#welcome > .settings-button")
        .addEventListener("click", () => {
            set("first-time", false);
            g("welcome").close();
            toggleSettings();
