...
function applyShortcutData(shortcutData, id) {
    ...
    if (shortcut) {
        ...
        shortcut.querySelector(".shortcut-emoji").innerText =
            shortcutData.emoji;
...
document.querySelectorAll(".shortcut").forEach((shortcut) => {
    shortcut.addEventListener("contextmenu", (e) => {
        ...
        if (
            saved("shortcut-" + window.editingShortcut) &&
            JSON.parse(get("shortcut-" + window.editingShortcut))
        ) {
            const data = JSON.parse(get("shortcut-" + window.editingShortcut));
            el.shortcutModal.querySelector("#shortcut-name").value = data.label;
            el.shortcutModal.querySelector("input[type='url']").value =
                data.url;
            el.shortcutModal.querySelector("#shortcut-emoji").value =
                data.emoji;
        }
